...
版本規則：每次網站內容更新時，第三碼加 1（例如 `V1.1.0` → `V1.1.1`）。每次部署到雲端前，先詢問是否將第二碼加 1；若同意則使用下一個次版本號（例如 `V1.1.1` → `V1.2.0`），若不同意則維持第二碼。
...
qa.html                    Q&A 公開頁面
data/qa.json               Q&A 問題、答案、分類與發布狀態
docker-compose.staging.yml  Directus + PostgreSQL 本機測試環境
docs/staging-cms.md         CMS、Google 登入與正式環境切換步驟
```
...
產品也可以在管理後台設定「適用市場」：選擇「大陸專屬」時，產品只會在簡體中文介面顯示；選擇「全球機種」時，只會在英文與繁體中文介面顯示；兩者都選時，三種語言都會顯示。既有產品若沒有 `audiences` 欄位，會視為兩者皆可見。

### 維護 Q&A

登入 `admin/` 後開啟「Q&A」分頁即可新增、編輯、刪除與停用問答。每筆問答包含英文、簡體中文、繁體中文的問題與答案，可設定分類、排序及是否發布；儲存後會直接更新 `data/qa.json`，GitHub Pages 重新部署後前台 Q&A 頁面即可看到變更。
